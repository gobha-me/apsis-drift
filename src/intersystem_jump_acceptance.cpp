...
[[nodiscard]] auto run_pilot_arrival(IntersystemJumpAlignmentState alignment,
                                     bool persist_at_commit)
  ...
  if (!document.state.intersystem_contract) {
    return std::unexpected{IntersystemJumpAcceptanceError::transition_failure};
  }
  ...
  if (resumed.travel_phase != IntersystemTravelPhase::target_system_flight ||
...
auto run_intersystem_jump_acceptance(int width, int height)
  ...
  const auto encoded = encode_save_document_json(document);
  if (!encoded) {
    return std::unexpected{IntersystemJumpAcceptanceError::persistence_failure};
  }
  ...
  for (SimulationTick tick = kJumpTransitTicks / 2; tick < kJumpTransitTicks;
       ++tick) {
    if (!advance_intersystem_jump_tick(resumed, target)) {
      return std::unexpected{
          IntersystemJumpAcceptanceError::transition_failure};
  ...
  const auto pilot_aligned =
      run_pilot_arrival(IntersystemJumpAlignmentState{}, false);
  const auto pilot_offset = run_pilot_arrival(
      IntersystemJumpAlignmentState{.heading_error_millidegrees = 30'000,
                                    .velocity_error_basis_points = 1'000,
                                    .controls = {}},
      true);
  const auto pilot_opposed = run_pilot_arrival(
      IntersystemJumpAlignmentState{.heading_error_millidegrees = 90'000,
  ...
  return IntersystemJumpAcceptanceResult{
      .report =
          IntersystemJumpAcceptanceReport{
              ...
              .assisted_quality = resumed.arrival_solution->assessment->quality,
...
auto intersystem_jump_acceptance_json(
  ...
  return std::format(
      ...
      "}}\n",
      system_id_string(report.destination), report.reference_planet.value,
      report.committed_tick, report.arrival_tick, report.arrival_checksum,
