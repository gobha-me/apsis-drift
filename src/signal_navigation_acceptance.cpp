...
auto advance_signal_navigation_acceptance(
  ...
  if (state.navigation.status == SignalScannerStatus::reached) return true;
  ...
  const std::array command{FlightCommand{
      state.flight.tick, FlightCommandKind::press_forward}};
  const std::span<const FlightCommand> commands =
      state.flight.tick == 0 ? std::span<const FlightCommand>{command}
  ...
  state.navigation = *navigation;
  return state.navigation.status == SignalScannerStatus::reached;
}
...
auto replay_signal_navigation_acceptance()
  ...
  while (state->navigation.status != SignalScannerStatus::reached) {
    const auto reached =
...
auto signal_navigation_acceptance_json(
  ...
  return std::format(
      ...
      report.command_count, report.final_distance_metres,
