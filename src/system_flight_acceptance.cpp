...
[[nodiscard]] auto replay(int width, int height, int render_interval)
  ...
  auto contract =
      initial_intersystem_contract_state(Seed{kSystemFlightAcceptanceSeed});
  const auto system =
...
auto run_system_flight_acceptance(int width, int height)
  ...
  if (at_30->contract != at_60->contract || at_30->flight != at_60->flight ||
      at_30->orbital != at_60->orbital || at_30->frame != at_60->frame) {
    return std::unexpected{SystemFlightAcceptanceError::cadence_mismatch};
  }
  return SystemFlightAcceptanceResult{
      .report = {.system = at_30->flight.system,
                 .planet = at_30->flight.target,
                 .arrival_tick =
                     at_30->contract.arrival_solution
...
auto system_flight_acceptance_json(const SystemFlightAcceptanceReport& report)
    -> std::string {
  return std::format(
      "{{\n"
      "  \"schema_version\": 2,\n"
      ...
      "}}\n",
      system_id_string(report.system), report.planet.value, report.arrival_tick,
      report.insertion_tick, report.host_steps, report.system_flight_checksum,
      report.orbital_flight_checksum, report.framebuffer_checksum);
}
