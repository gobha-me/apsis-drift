...
inline auto validate(const Request& request) -> void {
  if (request.samples < 2 || request.samples > 513 ||
      ...
      std::abs(request.longitude) > std::numbers::pi || request.lod > 10 ||
      request.relief_version > kExperimentalReliefVersion) {
    throw std::invalid_argument(
        "invalid snapshot dimensions, coordinates or LOD");
  }
}
...
inline auto require(std::expected<T, E> result) -> T {
  if (!result)
    throw std::runtime_error("C++ world API rejected snapshot request");
...
inline auto snapshot(const Request& request) -> nlohmann::json {
  ...
  for (unsigned row = 0; row < request.samples; ++row) {
    for (unsigned column = 0; column < request.samples; ++column) {
      ...
      const auto probe =
          require(planet_fixed_from_local(frame, {east, north, 0}));
      const auto sample = with_relief(require(sampler.sample(probe)), planet,
                                      probe, request.relief_version);
    ...
    if (tick % 4 == 0) {
      ...
      replay.push_back(
          {{"tick", flight.tick},
           ...
           {"checksum",
            std::to_string(planetary_flight_state_checksum(flight))}});
    }
    if (tick == 1200) break;
    const auto probe =
        require(planet_fixed_from_geodetic(planet, flight.pose.position));
    const auto surface = with_relief(require(sampler.sample(probe)), planet,
                                     probe, request.relief_version);
  ...
  nlohmann::json result = {
      ...
      {"lod", request.lod},
      {"tiles_touched", sampler.tiles_touched()},
      {"elevation_min_metres", low},
      {"elevation_max_metres", high},
      {"vertices", std::move(vertices)},
      {"colors", std::move(colors)},
      {"simulation_hz", kSimulationHz},
