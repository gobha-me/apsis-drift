...
inline auto relief_noise(Seed seed, const std::array<double, 3>& p,
  ...
  for (unsigned i = 0; i < 8; ++i) {
    ...
    for (unsigned axis = 0; axis < 3; ++axis)
      hash = relief_mix(hash ^ static_cast<std::uint64_t>(cell[axis] + ((i >> axis) & 1U)));
  ...
  return lerp(lerp(lerp(corner[0], corner[1], weight[0]),
              ...
              lerp(lerp(corner[4], corner[5], weight[0]),
                   lerp(corner[6], corner[7], weight[0]), weight[1]), weight[2]);
}

inline auto relief_offset(Seed planet_seed, std::array<double, 3> reference_position,
  ...
  const auto seed = derive_terrain_generation_seed(planet_seed, TerrainGenerationStream::detail);
  ...
  for (unsigned axis = 0; axis < 3; ++axis) {
    ...
    reference_position[axis] += (relief_noise(warp_seed, reference_position, 9500) - 0.5) * 1600;
  }
  ...
  for (unsigned octave = 0; octave < wavelengths.size(); ++octave) {
    ...
    const double ridge = 1.0 - std::abs(2.0 * relief_noise(octave_seed, reference_position,
                                                       wavelengths[octave]) - 1.0);
...
inline auto with_relief(TerrainSurfaceSample sample, const PlanetDescriptor& planet,
                         PlanetFixedPositionMetres position, unsigned version)
    -> TerrainSurfaceSample {
  if (version == 0) return sample; // Exact original v1 path, including arithmetic.
  auto coordinate = geodetic_from_planet_fixed(planet, position);
  if (!coordinate) throw std::invalid_argument("invalid relief surface direction");
  coordinate->altitude_metres = 0;
  const auto reference = planet_fixed_from_geodetic(planet, *coordinate);
  if (!reference) throw std::invalid_argument("invalid relief reference sphere");
  sample.elevation_metres += relief_offset(planet.seed,
      {reference->x, reference->y, reference->z}, version);
  return sample;
}
} // namespace apsis_drift::godot_spike
