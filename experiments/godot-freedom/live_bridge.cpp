...
struct LiveWorld {
  ...
  auto advance(double elapsed, unsigned buttons,
               std::optional<PlanetaryAnalogInput> analog = std::nullopt) -> void {
    if (lab) throw std::invalid_argument("use thrust input for flight lab session");
    ...
    for (int step = 0; step < scheduled.steps; ++step) {
      ...
      const auto sample =
          with_relief(require(sample_planet_surface(planet, probe, lod, cache)),
                      planet, probe, relief_version);
      if (!advance_planetary_flight(planet, {sample.elevation_metres}, flight,
                                    std::span{commands.data(), analog ? 0 : count},
                                    kSimulationStep, {}, analog))
  ...
  auto advance_thrust(double elapsed, flight_lab::Demand demand) -> void {
    ...
    const auto scheduled=require(candidate_clock.advance(SimulationSeconds{elapsed}));
    for (int i=0;i<scheduled.steps;++i) {
      const PlanetFixedPositionMetres fixed{candidate.position.x,candidate.position.y,candidate.position.z};
      const auto sample=with_relief(require(sample_planet_surface(planet,fixed,lod,cache)),planet,fixed,relief_version);
      flight_lab::advance(planet,sample.elevation_metres,candidate,demand);
    }
    const auto pose=require(geodetic_from_planet_fixed(planet,
    ...
    const auto east=flight_lab::vec(tangent.east),north=flight_lab::vec(tangent.north),up=flight_lab::vec(tangent.up);
    auto projected=flight;
    projected.pose.position=pose;
    if (std::hypot(flight_lab::dot(forward,east),flight_lab::dot(forward,north))>1e-8)
      projected.pose.heading_radians=std::atan2(flight_lab::dot(forward,north),flight_lab::dot(forward,east));
    projected.velocity={flight_lab::dot(candidate.velocity,east),flight_lab::dot(candidate.velocity,north),flight_lab::dot(candidate.velocity,up)};
    projected.tick=candidate.tick;
    projected.clearance_metres=candidate.clearance;
    // Legacy-shaped view is ONLY for terrain/camera adapters, never legacy saves.
    flight=projected;lab=candidate;clock=candidate_clock;
    dropped_seconds+=scheduled.dropped.count();
    previous_buttons=(demand.main>0?1:0)|(demand.retro>0?2:0)|
        (demand.yaw<0?4:0)|(demand.yaw>0?8:0)|(demand.strafe<0?16:0)|
        (demand.strafe>0?32:0)|(demand.heave>0?64:0)|(demand.heave<0?128:0);
  }
};
...
protected:
  static auto _bind_methods() -> void {
    ...
    godot::ClassDB::bind_method(godot::D_METHOD("enable_thrust_flight"), &FreedomBridge::enable_thrust_flight);
    godot::ClassDB::bind_method(godot::D_METHOD("get_sky_catalog"), &FreedomBridge::get_sky_catalog);
    godot::ClassDB::bind_method(godot::D_METHOD("start_practice", "reentry"), &FreedomBridge::start_practice);
    godot::ClassDB::bind_method(godot::D_METHOD("camera_clearance", "offset"), &FreedomBridge::camera_clearance);
    godot::ClassDB::bind_method(godot::D_METHOD("advance_thrust", "elapsed", "axes", "assist"), &FreedomBridge::advance_thrust);
...
public:
  auto get_sky_catalog() const -> godot::Array {
    ...
    for(const auto& star:flight_lab::sky_catalog(world->planet.seed)) {
      ...
      item["system_seed"]=godot::String(std::to_string(star.system_seed.value).c_str());
      item["direction"]=godot::Vector3(star.direction.x,star.direction.y,star.direction.z);
      item["color"]=godot::Color(star.color.red/255.0,star.color.green/255.0,star.color.blue/255.0);
  ...
  auto advance_analog(double elapsed, const godot::PackedFloat64Array &axes)
    ...
    try {
      ...
      const unsigned buttons = (axes[0] > 0 ? 1U : 0U) |
          (axes[0] < 0 ? 2U : 0U) | (axes[1] < 0 ? 4U : 0U) |
          (axes[1] > 0 ? 8U : 0U) | (axes[2] < 0 ? 16U : 0U) |
          (axes[2] > 0 ? 32U : 0U) | (axes[3] > 0 ? 64U : 0U) |
          (axes[3] < 0 ? 128U : 0U);
  ...
  auto enable_thrust_flight() -> bool {
    try {
      if (!world || world->flight.tick!=0 || world->lab)
        throw std::invalid_argument("enable flight lab once, before the first tick");
      world->lab=flight_lab::initial(world->planet,world->flight.pose.position,world->flight.pose.heading_radians);
      world->lab->clearance=world->flight.clearance_metres;
      last_error=godot::String{};return true;
    } catch (const std::exception& e) { last_error=e.what();return false; }
  }

  auto advance_thrust(double elapsed,const godot::PackedFloat64Array& axes,bool assist) -> bool {
    try {
      if (!world || axes.size()!=7) throw std::invalid_argument("expected seven flight actuator demands");
      world->advance_thrust(elapsed,{axes[0],axes[1],axes[2],axes[3],axes[4],axes[5],axes[6],assist});
      last_error=godot::String{};return true;
    } catch (const std::exception& e) { last_error=e.what();return false; }
  }
  ...
  auto get_state() const -> godot::Dictionary {
    ...
    if (world->lab) {
      ...
      const auto view_frame=stream ? make_local_tangent_frame(world->planet,flight.pose.position)
                                   : std::expected<LocalTangentFrame,CoordinateError>{world->frame};
      if (!view_frame) return {};
      godot::Basis basis;
      int column=0;
      for (auto axis : {lab.right,lab.up,lab.back})
        basis.set_column(column++,godot::Vector3(flight_lab::dot(axis,flight_lab::vec(view_frame->east)),
      ...
      result["periapsis"]=orbit.periapsis;result["apoapsis"]=orbit.apoapsis;
      result["bound_orbit"]=orbit.bound;result["clear_orbit"]=orbit.clear_orbit;
      ...
      result["body_velocity"]=godot::Vector3(flight_lab::to_body(lab,lab.velocity).x,
          flight_lab::to_body(lab,lab.velocity).y,flight_lab::to_body(lab,lab.velocity).z);
      godot::Basis sky_basis;
      sky_basis.set_column(0,godot::Vector3(view_frame->east.x,view_frame->east.y,view_frame->east.z));
      sky_basis.set_column(1,godot::Vector3(view_frame->up.x,view_frame->up.y,view_frame->up.z));
      sky_basis.set_column(2,godot::Vector3(-view_frame->north.x,-view_frame->north.y,-view_frame->north.z));
      result["view_to_inertial"]=sky_basis;
      result["main_thrust"]=lab.main;result["retro_thrust"]=lab.retro;
      result["air_density"]=lab.density;result["dynamic_pressure"]=lab.dynamic_pressure;
      result["acceleration"]=lab.acceleration;result["floor_guard"]=lab.floor_guard;
      result["assist"]=lab.assist;
      result["climb_rate"]=flight.velocity.up_metres_per_second;
      result["rcs_acceleration"]=godot::Vector3(lab.thrust_body.x,lab.thrust_body.y,lab.thrust_body.z);
      result["checksum"]=godot::String{("lab1:"+std::to_string(flight_lab::checksum(lab))).c_str()};
    }
    return result;
  }
  ...
  auto camera_clearance(godot::Vector3 offset) -> double {
    try {
      ...
      const auto frame=require(make_local_tangent_frame(world->planet,world->flight.pose.position));
      const auto fixed=require(planet_fixed_from_local(frame,{offset.x,-offset.z,offset.y}));
      const auto pose=require(geodetic_from_planet_fixed(world->planet,fixed));
      const auto sample=with_relief(require(sample_planet_surface(world->planet,fixed,world->lod,world->cache)),world->planet,fixed,world->relief_version);
      last_error=godot::String{};
      return pose.altitude_metres-sample.elevation_metres;
    } catch(const std::exception& e) {
      last_error=e.what();return std::numeric_limits<double>::quiet_NaN();
    }
  }
  ...
  auto start_practice(bool reentry) -> bool {
    ...
    if (!world || !world->lab || !stream) {
      last_error="Practice requires streaming thrust flight";return false;
    }
    ...
    s.assist=false;
    s.density=flight_lab::density(world->planet,reentry?90000.0:250000.0);
    s.dynamic_pressure=.5*s.density*flight_lab::dot(s.velocity,s.velocity);
