...
auto main() -> int {
  try {
    ...
    for (double invalid : {-1.00001, 1.00001,
      ...
      for (int axis = 0; axis < 4; ++axis) {
        ...
        check(!advance_planetary_flight(planet, {0}, state, {}, kSimulationStep,
                   {}, PlanetaryAnalogInput{values[0], values[1], values[2], values[3]}),
              "invalid axis accepted");
        check(state == *initial, "invalid axis mutated state");
      }
    }
    const std::array press{FlightCommandKind::press_forward,
                           FlightCommandKind::press_turn_right,
                           FlightCommandKind::press_strafe_right,
                           FlightCommandKind::press_rise};
    const std::array negative{FlightCommandKind::press_backward,
    ...
    for (int code = 0; code < 81; ++code) {
      ...
      for (int tick = 0; tick < 120; ++tick) {
        check(advance_planetary_flight(planet, {0}, digital,
                  {commands.data(), tick == 0 ? count : 0}, kSimulationStep).has_value(),
              "digital step rejected");
        check(advance_planetary_flight(planet, {0}, analog, {}, kSimulationStep,
                  {}, PlanetaryAnalogInput{axes[0], axes[1], axes[2], axes[3]}).has_value(),
        ...
        check(planetary_flight_state_checksum(analog) ==
                  planetary_flight_state_checksum(digital), "endpoint checksum");
      }
    }
    ...
    for (int i = 0; i < 120; ++i) {
      for (auto* state : {&partial, &duplicate})
        check(advance_planetary_flight(planet, {0}, *state, {}, kSimulationStep,
                  {}, PlanetaryAnalogInput{0.0001, 0.25, 0, 0}).has_value(), "fractional step");
      check(advance_planetary_flight(planet, {0}, full, {}, kSimulationStep,
                  {}, PlanetaryAnalogInput{1, 1, 0, 0}).has_value(), "full step");
    }
    check(partial == duplicate, "fractional input not deterministic");
    check(partial.pose.heading_radians != full.pose.heading_radians &&
              partial.velocity.east_metres_per_second != full.velocity.east_metres_per_second,
    ...
    check(!advance_planetary_flight(planet, {0}, mixed, {&command, 1},
              kSimulationStep, {}, PlanetaryAnalogInput{}), "ambiguous mixed input accepted");
    check(mixed == *initial, "mixed input mutated state");
    std::cout << "Analog flight contract: invalid axes, 81 endpoint combinations, "
