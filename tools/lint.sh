...
build_dir=${APSIS_DRIFT_TIDY_BUILD_DIR:-build-tidy}
jobs=${APSIS_DRIFT_TIDY_JOBS:-2}
...
if [[ ! "$jobs" =~ ^[1-9][0-9]*$ ]]; then
  echo "error: APSIS_DRIFT_TIDY_JOBS must be a positive integer" >&2
  exit 2
fi
...
tools/check_nolint.sh --self-test
